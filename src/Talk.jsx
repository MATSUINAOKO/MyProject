...
export default function Talk(props) {
  const {setCurrentView,message,
    input, setInput,messages, setMessages, clickedIndex, setClickedIndex}=props
...
//fetchで送信
  const response = await fetch(`/api?${queryParams}`,//エンドポイントに上で作成したurlをつける
  {method : "POST",
  headers: {
    'Content-Type': 'application/json', // ヘッダーにContent-Typeを指定（ないとエラー）
  },
  body:JSON.stringify(data)} //req.bodyの中身
  )
    try{
    ...
    try {
      ...
      if (data.status === 0) {
        ...
        const postBotMessage ={ talk: tolkWord, user_name: 'potato', user_id: 6}
          await setMessages(messages => [...messages, { talk: tolkWord, user_name: 'potato' }]);//画面表示変更
